...
func _validate_manifest(manifest: Dictionary) -> Array[Dictionary]:
	...
	if _string_value(manifest.get("registry_id", "")) != REGISTRY_ID:
		findings.append(_finding(
			"error",
			"registry_id",
			"Expected registry id " + REGISTRY_ID,
		))
	if not _positive_int(manifest.get("content_version", -1)):
		findings.append(_finding(
			"error",
			"content_version",
	...
	if not forbidden_value is Array or (forbidden_value as Array).is_empty():
		...
		for marker_value in forbidden_value:
			var marker := _string_value(marker_value)
			if marker.is_empty():
				findings.append(_finding(
					"error",
	...
	if not blocker_value is Array:
		...
		for blocker_entry_value in blocker_value:
			...
			var blocker_id := _string_value(blocker.get("id", ""))
	...
	for entry_value in assets_value:
		...
		var availability := _string_value(entry.get("availability", ""))
		...
		var runtime_hash := _string_value(entry.get("runtime_sha256", ""))
		if availability == AVAILABILITY_PENDING_UNIMPORTED:
			if not runtime_path.is_empty() or not runtime_hash.is_empty():
				findings.append(_finding(
					"error",
					"pending_runtime",
					"Pending assets cannot claim a runtime path or runtime hash",
					asset_id,
				))
			findings.append(_finding(
				"warning",
			...
			if not runtime_path.begins_with("res://assets/") or runtime_path.find("..") >= 0 or _contains_forbidden(runtime_path, forbidden_paths):
				findings.append(_finding(
					"error",
					"runtime_path",
		...
		if not provenance_value is Dictionary:
			...
			var source_relative_path := _string_value(
				provenance.get("source_relative_path", "")
			)
			var source_status := _string_value(provenance.get("source_status", ""))
			var source_hash := _string_value(provenance.get("source_sha256", ""))
			if source_root.is_empty() or source_relative_path.is_empty() or source_relative_path.begins_with("/") or source_relative_path.find("..") >= 0:
				findings.append(_finding(
					"error",
					"provenance_path",
					"Source root and relative path must be explicit and confined",
					asset_id,
				))
			...
			if not source_hash.is_empty() and not _is_sha256(source_hash):
				findings.append(_finding(
					"error",
					"source_hash",
					"Source SHA-256 must be null or a 64-character hex digest",
					asset_id,
				))
			if availability == AVAILABILITY_IMPORTED and source_status != "available":
				findings.append(_finding(
					"error",
					"source_status",
			...
			if availability == AVAILABILITY_IMPORTED_SOURCE_MISSING:
				if source_status != "missing" and source_status != "unavailable_external":
					findings.append(_finding(
						"error",
						"source_status",
		...
		var family := _string_value(entry.get("family", ""))
		var kind := _string_value(entry.get("kind", ""))
		...
		if not filtering_value is Dictionary:
			...
			var mode := _string_value(filtering.get("mode", ""))
		...
		if not license_value is Dictionary:
			...
			var license_status := _string_value(license.get("status", ""))
			var license_reference := _string_value(license.get("reference", ""))
			...
			elif license_reference.begins_with("res://") and not FileAccess.file_exists(license_reference):
				findings.append(_finding(
					"error",
					"license_reference_missing",
					"Referenced license file does not exist",
					asset_id,
				))
			...
			if license_status == LICENSE_BLOCKED and _string_value(license.get("reason", "")).is_empty():
				findings.append(_finding(
					"error",
					"license_reason",
		...
		if not links_value is Array:
			...
			for link_value in links_value:
				var link := _string_value(link_value)
				if not link.begins_with("zerkov.") or link.find(" ") >= 0:
					findings.append(_finding(
						"error",
						"content_link",
						"Content links must be explicit zerkov.* identifiers",
						asset_id,
					))
...
func _validate_atlas(value: Variant, asset_id: String) -> Array[Dictionary]:
	var findings: Array[Dictionary] = []
	if not value is Dictionary:
		findings.append(_finding(
			"error",
	...
	var columns_value: Variant = atlas.get("columns", 0)
	var rows_value: Variant = atlas.get("rows", 0)
	var frame_count_value: Variant = atlas.get("frame_count", 0)
	if not _positive_int(columns_value) or not _positive_int(rows_value) or not _positive_int(frame_count_value):
		findings.append(_finding(
			"error",
			"atlas_frames",
			"Atlas columns, rows, and frame_count must be positive integers",
			asset_id,
		))
	else:
	...
	if order_value != null:
		if not order_value is Array:
		...
		elif _positive_int(frame_count_value) and (order_value as Array).size() != int(frame_count_value):
			findings.append(_finding(
				"error",
			...
			for frame_value in order_value:
				if not _nonnegative_int(frame_value) or int(frame_value) >= int(frame_count_value):
					findings.append(_finding(
						"error",
...
func _is_positive_pair(value: Variant) -> bool:
	if not value is Array or (value as Array).size() != 2:
		return false
	return _positive_int((value as Array)[0]) and _positive_int((value as Array)[1])
...
func _contains_forbidden(value: String, forbidden_paths: Array) -> bool:
	...
	for marker_value in forbidden_paths:
		var marker := _string_value(marker_value).to_lower()
