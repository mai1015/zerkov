#ifndef INVENTORY_SYSTEM_CORE_BYTES_H
#define INVENTORY_SYSTEM_CORE_BYTES_H
...
// Canonical integers are explicitly little-endian. Readers validate every
// count and byte range before callers reserve or construct canonical state.
namespace inv {
...
public:
	...
	void write_count(std::size_t p_count, std::size_t p_limit = MAX_COLLECTION_COUNT) {
		if (p_count > p_limit || p_count > MAX_COLLECTION_COUNT ||
				p_count > static_cast<std::size_t>(UINT32_MAX)) {
...
public:
	...
	bool read_count(std::size_t &r_count, std::size_t p_limit = MAX_COLLECTION_COUNT, std::size_t p_min_bytes_per_entry = 0) {
		std::uint32_t raw = 0;
		if (!read_u32(raw)) {
			return false;
		}
...
#endif // INVENTORY_SYSTEM_CORE_BYTES_H
