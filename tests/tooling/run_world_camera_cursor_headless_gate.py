#!/usr/bin/env python3
"""Run the world camera cursor contract at the exact first-playable output."""
...
GODOT = os.environ.get("ZERKOV_GODOT", os.environ.get("GODOT_BIN", "godot"))
CONTRACT = "res://tests/presentation/world_camera_cursor_contract.gd"
RESULT = re.compile(r"WORLD_CAMERA_CURSOR_RESULT checks=(\d+) failures=(\d+)")
...
def main() -> int:
    ...
    for index, (_, output, _, _, diagnostics) in enumerate(runs, start=1):
        print(f"--- World Camera Cursor contract run {index} ---")
    ...
    print(
        "WORLD_CAMERA_CURSOR_HEADLESS_GATE"
