#!/usr/bin/env python3
"""Run the movement authority contract at the exact first-playable output."""
...
CONTRACT = "res://tests/raid/movement_authority_contract.gd"
RESULT = re.compile(r"MOVEMENT_AUTHORITY_RESULT checks=(\d+) failures=(\d+)")
...
def main() -> int:
    ...
    for index, (_, output, _, _, diagnostics) in enumerate(runs, start=1):
        print(f"--- Movement Authority contract run {index} ---")
    ...
    print(
        "MOVEMENT_AUTHORITY_HEADLESS_GATE"
