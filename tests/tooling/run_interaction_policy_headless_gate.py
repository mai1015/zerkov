#!/usr/bin/env python3
"""Run the interaction policy contract at the exact first-playable output."""
...
CONTRACT = "res://tests/raid/interaction_policy_contract.gd"
RESULT = re.compile(r"INTERACTION_POLICY_RESULT checks=(\d+) failures=(\d+)")
...
def main() -> int:
    ...
    for index, (_, output, _, _, diagnostics) in enumerate(runs, start=1):
        print(f"--- Interaction Policy contract run {index} ---")
    ...
    print(
        "INTERACTION_POLICY_HEADLESS_GATE"
