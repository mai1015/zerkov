#!/usr/bin/env python3
"""Run the player locomotion contract at the exact first-playable output."""
...
CONTRACT = "res://tests/raid/player_locomotion_contract.gd"
RESULT = re.compile(r"PLAYER_LOCOMOTION_RESULT checks=(\d+) failures=(\d+)")
...
def main() -> int:
    ...
    for index, (_, output, _, _, diagnostics) in enumerate(runs, start=1):
        print(f"--- Player Locomotion contract run {index} ---")
    ...
    print(
        "PLAYER_LOCOMOTION_HEADLESS_GATE"
