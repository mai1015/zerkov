#!/usr/bin/env python3
"""Run the navigation path contract at the exact first-playable output."""
...
CONTRACT = "res://tests/raid/navigation_path_contract.gd"
RESULT = re.compile(r"NAVIGATION_PATH_RESULT checks=(\d+) failures=(\d+)")
...
def main() -> int:
    ...
    for index, (_, output, _, _, diagnostics) in enumerate(runs, start=1):
        print(f"--- Navigation path contract run {index} ---")
