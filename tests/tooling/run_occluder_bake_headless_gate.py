#!/usr/bin/env python3
"""Run the Common Vision occluder bake contract at the exact first-playable output."""
...
CONTRACT = "res://tests/ai/occluder_bake_contract.gd"
RESULT = re.compile(r"OCCLUDER_BAKE_RESULT checks=(\d+) failures=(\d+)")
...
def main() -> int:
    ...
    for index, (_, output, _, _, diagnostics) in enumerate(runs, start=1):
        print(f"--- Occluder Bake contract run {index} ---")
