#!/usr/bin/env python3
"""Run the world markers contract at the exact first-playable output."""
...
CONTRACT = "res://tests/raid/world_markers_contract.gd"
RESULT = re.compile(r"WORLD_MARKERS_RESULT checks=(\d+) failures=(\d+)")
...
def main() -> int:
    ...
    for index, (_, output, _, _, diagnostics) in enumerate(runs, start=1):
        print(f"--- World Markers contract run {index} ---")
