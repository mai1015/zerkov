...
func _test_overlap_and_tie_policy() -> void:
	...
	var actor_tie := world.raycast(_query(
	...
	var torso_seam := world.raycast(_query(
	...
	var limb_seam := world.raycast(_query(
	...
	var rational := world.raycast(_query(
	...
	var reverse := world.raycast(_query(
...
func _test_obstruction_policy() -> void:
	...
	var tie := (tie_context["world"] as BodyHitboxWorld2D).raycast(_query(
	...
	var lexical := (lexical_context["world"] as BodyHitboxWorld2D).raycast(_query(
	...
	var behind := (behind_context["world"] as BodyHitboxWorld2D).raycast(_query(
	...
	var ahead := (ahead_context["world"] as BodyHitboxWorld2D).raycast(_query(
	...
	var masked := (masked_context["world"] as BodyHitboxWorld2D).raycast(_query(
	...
	var boundary := (boundary_context["world"] as BodyHitboxWorld2D).raycast(_query(
...
func _test_snapshot_adversaries() -> void:
	...
	check(world.publish_snapshot(0, 1, [initial], [], generation, token),
	...
	check(not world.publish_snapshot(0, 1, [initial, added_at_equal], [], generation, token)
	...
	check(world.publish_snapshot(1, 2, [initial], [], generation, token),
	...
	check(world.publish_snapshot(2, 3, [], [], generation, token),
	...
	check(not world.publish_snapshot(3, 4, [initial], [], generation, token)
	...
	check(world.publish_snapshot(3, 4, [readded], [], generation, token),
	...
	check(not world.publish_snapshot(4, 5, [changed_equal], [], generation, token)
	...
	check(world.publish_snapshot(4, 5, [changed_new], [], generation, token),
		"new body revision admits the explicit geometry update")
	check(not world.publish_snapshot(4, 7, [changed_new], [], generation, token)
...
func _test_query_adversaries() -> void:
	...
	var valid := _query(
		...
		Vector2i(10_000, -600_000), BODY_LAYER, 0)
	check(world.raycast(7).get("reason") == &"ray_query_type_invalid",
		"non-dictionary query fails closed")
	var malformed := valid.duplicate(true)
	malformed["extra"] = true
	check(world.raycast(malformed).get("reason") == &"ray_query_schema_invalid",
		"query rejects unknown keys")
	malformed = valid.duplicate(true)
	malformed["origin_raw"] = Vector2(0.0, -600_000.0)
	check(world.raycast(malformed).get("reason") == &"ray_query_field_type_invalid",
		"floating query geometry is rejected")
	malformed = valid.duplicate(true)
	malformed["request_id"] = "forged"
	check(world.raycast(malformed).get("reason") == &"ray_query_request_id_invalid",
		"forged query identity is rejected")
	malformed = valid.duplicate(true)
	malformed["body_mask"] = 0
	check(world.raycast(malformed).get("reason") == &"ray_query_mask_invalid",
		"query with no collision domain is rejected")
	malformed = valid.duplicate(true)
	malformed["target_raw"] = malformed["origin_raw"]
	check(world.raycast(malformed).get("reason") == &"ray_query_geometry_invalid",
	...
	check(world.raycast(malformed).get("reason")
	...
	check(world.raycast(malformed).get("reason") == &"ray_query_exclusion_duplicate",
	...
	check(world.raycast(malformed).get("reason") == &"authority_generation_mismatch",
		"forged authority generation is rejected")
	malformed = valid.duplicate(true)
	malformed["binding_token"] = int(context["token"]) + 1
	check(world.raycast(malformed).get("reason") == &"binding_token_mismatch",
		"forged binding token is rejected")
	malformed = valid.duplicate(true)
	malformed["world_revision"] = world.snapshot_revision() + 1
	check(world.raycast(malformed).get("reason") == &"world_revision_mismatch",
		"future world revision is rejected")
	malformed = valid.duplicate(true)
	malformed["tick"] = world.snapshot_tick() + 1
	check(world.raycast(malformed).get("reason") == &"query_tick_mismatch",
	...
	var first_result := world.raycast(first_order)
	var second_result := world.raycast(second_order)
	...
	var packed_result := world.raycast(packed_order)
...
func _test_obstruction_revision_lifecycle() -> void:
	...
	check(world.publish_snapshot(0, 1, [], [original], generation, token),
	...
	check(world.publish_snapshot(1, 2, [], [], generation, token),
		"complete snapshot omission removes obstruction")
	check(authority.advance_one(generation),
		"obstruction revision fixture advances to re-add")
	check(not world.publish_snapshot(2, 3, [], [original], generation, token)
	...
	check(world.publish_snapshot(2, 3, [], [renewed], generation, token),
	...
	check(not world.publish_snapshot(3, 4, [], [changed_equal], generation, token)
	...
	check(world.publish_snapshot(3, 4, [], [changed_new], generation, token),
...
func _test_lifecycle_and_rebind() -> void:
	...
	check(world.publish_snapshot(0, 1, [_body(target, 1, Vector2i.ZERO)], [],
		generation, first_token), "lifecycle snapshot publishes")
	var old_query := _query(
		context, "lifecycle_old", Vector2i(0, -600_000),
		Vector2i(10_000, -600_000), BODY_LAYER, 0)
	check(world.raycast(old_query).get("reason")
	...
	check(bool(world.raycast(old_query).get("hit", false)),
	...
	check(world.raycast(settling_query).get("reason")
		== &"authority_not_accepting_world_query",
		"settling authority rejects late combat queries")
	check(world.release_binding(first_token),
		"binding releases after its authority becomes terminal to combat")
	check(world.snapshot_metadata().is_empty(),
	...
	check(world.bind_raid_authority(replacement_authority, generation),
	...
	check(world.publish_snapshot(0, 1, [_body(target, 2, Vector2i.ZERO)], [],
		generation, replacement_token), "replacement snapshot publishes")
	check(replacement_authority.transition(RaidAuthority.Lifecycle.ACTIVE, generation),
		"replacement authority activates")
	check(world.raycast(old_query).get("reason") == &"binding_token_mismatch",
		"late query captured from old binding cannot reach replacement state")
	...
	var replacement_query := _query(
		...
		Vector2i(10_000, -600_000), BODY_LAYER, 0)
	check(bool(world.raycast(replacement_query).get("hit", false)),
	...
	check(world.raycast(late_query).get("reason") == &"binding_generation_invalidated",
		"authority teardown invalidates late world queries")
	check(not world.publish_snapshot(0, 1, [], [], generation, replacement_token)
		and world.last_error == &"binding_generation_invalidated",
		"authority teardown invalidates late snapshot publication")
	check(world.teardown(replacement_token),
...
func _test_query_capacity() -> void:
	...
	var overflow := world.raycast(_query(
	...
	var replay := world.raycast(_query(
...
func _check_failed_publication(
	...
	var accepted := world.publish_snapshot(
...
func _active_context(label: String, bodies: Array, obstructions: Array) -> Dictionary:
	...
	var authority := context["authority"] as RaidAuthority
	var world := context["world"] as BodyHitboxWorld2D
	check(world.publish_snapshot(0, 1, bodies, obstructions,
		int(context["generation"]), int(context["token"])),
