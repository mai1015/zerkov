...
func _exercise() -> void:
	...
	var added := native.insert_item(inventory_id, String(ZerkovInventoryCatalog.ITEM_AKM), 1,
		{"kind":"spatial", "container":pack.container_id, "x":0, "y":0, "rotated":false}, LocalCampaignContent.CONTENT_ACTOR)
	...
	_check(native.remove_item(inventory_id, added.new_item_id, LocalCampaignContent.CONTENT_ACTOR).accepted, "remove second firearm fixture")
	...
	for pair: Array in [[ZerkovInventoryCatalog.ITEM_RIG_BASIC, &"zerkov.slot.rig"],
		...
		if not _check(controller.submit_equip(&"backpack", other, pair[1]).accepted, "canonical rig/backpack equip"): return
		...
		_check(native.remove_item(inventory_id, added.new_item_id, LocalCampaignContent.CONTENT_ACTOR).accepted, "remove container gear fixture")
		...
		for y in range(int(desc.rows)):
			for x in range(int(desc.columns)):
				...
				var result := native.insert_item(inventory_id, String(ZerkovInventoryCatalog.ITEM_BANDAGE), 1,
					{"kind":"spatial", "container":desc.container_id, "x":x, "y":y, "rotated":false}, LocalCampaignContent.CONTENT_ACTOR)
	...
	for id: int in filler: _check(native.remove_item(inventory_id, id, LocalCampaignContent.CONTENT_ACTOR).accepted, "remove native occupancy fixture")
	item = test.call("equipment", PRIMARY)
	before = native.make_persistence_record(inventory_id)
