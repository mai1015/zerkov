...
func run(h, initial: Dictionary) -> bool:
	...
	if h._run_mode == "new":
		...
		h.check(not game._ui.screen.get_node("InventoryContent/DesktopStashScroll").visible and game._ui.screen.get_node("InventoryContent/LocalLootEmpty").visible, "closed raid loot has a real empty state")
		h.check(game._ui.screen.get_node("InventoryContent/LootTab").disabled, "loot tab cannot open the default crate")
