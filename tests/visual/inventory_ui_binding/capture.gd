...
func capture(state: String, extra: Dictionary = {}) -> void:
	await settle()
	# A native window can become idle between state changes, in which case
	# frame_post_draw may never fire. Force the pending draw before reading the
	# viewport so this promoted capture gate always terminates deterministically.
