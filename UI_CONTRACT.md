...
`const U = preload("res://ui/theme/tokens.gd")` is available in the base. Shared styles live in `ui/theme/zerkov_theme.tres`; project buttons and the border adapter use it. `app.state` is UI-only fixture state owned by `ui/dev/fixture_store.gd`, not gameplay authority. UI services are `app.navigate(route)`, `app.back()`, `app.toast(text)`, `app.confirm(title, message, callback)` and `app.prompt(...)`. Register explicit paths and roles in `ZRouteCatalog`; keep the 28 current route IDs stable. Ongoing authority integration is separate work.
