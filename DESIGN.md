...
## 4. Spacing & layout
1080p first-playable policy (2026-09-10): the approved 1920×1080 composition is the sole current UI and visual acceptance target. Play opens at 1920×1080, and screen geometry follows the exact reference coordinates. Existing responsive and compact code may remain unchanged as historical regression behavior, but smaller-output adaptation, capture review, and acceptance are deferred to a later proposal. New first-playable work must not add layout branches or reduce the authored desktop hierarchy for smaller windows.
