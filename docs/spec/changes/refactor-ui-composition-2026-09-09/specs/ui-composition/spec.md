...
Shared chrome and feature content SHALL own their desktop and compact layouts through repeatable layout APIs. Resizing MUST preserve active screen identity, and responsive placement MUST use explicit content references rather than inferred screen coordinates.

#### Scenario: A supported window changes layout mode
- **WHEN** the window transitions from desktop to compact and back
...
#### Scenario: A desktop window resizes within desktop mode
- **WHEN** its size stays at or above 1280×720 in automatic layout
...
The migration MUST preserve all 28 route identifiers and the established visual contract. Acceptance SHALL include focused lifetime/component regressions, the existing relevant UI and border suites, and native captures at declared desktop and compact sizes. Runtime errors MUST count as failures even when the engine process returns zero.

#### Scenario: A feature family finishes migration
- **WHEN** its old builders and resource paths are removed
- **THEN** its route, action, input, and compact tests pass
...
- **AND** native comparisons cover 1920×1080, 1600×900, 1280×720, 960×540, and supported forced-compact cases
