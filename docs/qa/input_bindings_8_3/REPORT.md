...
so Map retains M, and inventory uses I rather than Godot's Tab focus action.
...
explicit replace/allow-duplicate options and native protection checks. Request
IDs are reserved before synchronous native mutation/publication, and context
leases are distinct caller-owned capabilities with project-owned priorities.
...
| `zerkov_input_bindings_contract.gd` | `369` checks, `0` failures; exact-1920 runtime path |
