...
def baseline():
    ...
    write_json('reviewed_diff.json', {
        'encoding': 'base64',
        'description': 'Exact reviewed catalog diff encoded to avoid nested patch whitespace diagnostics.',
        'source_command': ['git', 'diff', '--', 'game/content/zerkov_inventory_catalog.gd'],
        'decoded_sha256': hashlib.sha256(changed.encode()).hexdigest(),
        'data': base64.b64encode(changed.encode()).decode('ascii'),
    })
